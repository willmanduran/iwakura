...
void handle_shutdown(int sig) {
    ...
    log_msg("Dashboard stopped cleanly. Goodbye!");
    if (log_file) fclose(log_file);

    printf("Dashboard stopped cleanly. Check ~/.iwakura/launcher.log for details.\n");
    exit(0);
}
...
int main() {
    system("mkdir -p $HOME/.iwakura");
    char log_path[512];
    snprintf(log_path, sizeof(log_path), "%s/.iwakura/launcher.log", getenv("HOME"));
    log_file = fopen(log_path, "a");

    log_msg("=== IWAKURA DASHBOARD STARTING ===");
    ...
    const char *lang = getenv("DASH_LANG");
    if (lang) {
        char lang_path[256];
    ...
    log_msg("Handing over terminal to Orchestrator...");
    system("./orch_main");
